...
template <std::size_t N, std::size_t... Indexes>
constexpr auto StringFactory(const char* value, std::index_sequence<Indexes...> dummy)
{
	return StringConstant<N>(value[Indexes]...);
}
...
template <std::size_t N>
constexpr auto StringFactory(const char* value)
{
